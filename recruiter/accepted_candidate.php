<?php 
	...
	if(isset($_POST['app_id']) && isset($_POST['candidate_app']) && isset($_POST['int_date']))
	{
		...
		$sql3="UPDATE applications SET status='$status',interview_date='$int_date' WHERE app_id IN('$app_id')";
		...
		<div class="row"><br><br>
			...
			<div class="">
				...
				<div class="tab-link1">								  
					...
					<a class="active">Accepted Candidates</a>
				...
				<div id="Accepted" class="tab-linkcontent1">
					...
					<div class="row">
						...
						<div class="col-lg-3 sidebar">
							<div class="single-slidebar">
								...
								<form action="accepted_candidate.php?id=<?php echo $rid; ?>" method="POST">
									...
									<div class="form-group">
										...
										<select class="form-control" id="select_loc" name="loc">
											...
											<?php 
												$sql="SELECT DISTINCT location,user_first,user_last FROM applications INNER JOIN user ON applications.user_id=user.user_id WHERE location!='' AND status='accepted'";
						...
						?>
							<h3 align="center">Accepted applicants for the post of <?php echo $title_row["job_title"]; ?></h3>
							<br>
							...
							<table class="table" align="center">
								...
								<?php  
									...
									{
										$sql="SELECT app_id,user.user_id,job_title,user_first,user_last,mobile,highest_qualification,course,specialization,percentage,interview_date FROM applications INNER JOIN user ON applications.user_id=user.user_id INNER JOIN joblist ON applications.job_id=joblist.job_id WHERE applications.recruiter_id='$SESSION' AND applications.job_id='$id' AND status='accepted' ORDER BY percentage DESC LIMIT $page1,5 ";
										$result = mysqli_query($conn,$sql);
									}
								...
								?>
									<tr id='trr'>
										<td id='tdd'><input type="checkbox" name="option" data-user="<?php echo $row["user_id"]; ?>" id="<?php echo $row["app_id"]; ?>"></td>
										...
										<td id='tdd'><?php echo $row["course"];?></td>
						...
						?>
							...
							<button type="submit" name="schedule" id="schedule">Schedule Interview</button>
							<input type="hidden" name="candidate_app" id="candidate_app">
								...
								<div class="modal-dialog">
									<div class="modal-content">
										...
										<div class="modal-body">      
											<div class="form-group">
												Set date for scheduled interview: <input type="date" class="" id="int_date" name="int_date" placeholder="">
											</div>
											<div class="form-group">
												<input type="button" class="" id="submit_date" name="submit_date" value="Submit Date">
											</div>
											<div id="disp_result" class="form-group"></div>
										</div>
									</div>
								</div>
							</div>
							...
							<script type="text/javascript">
								$('#schedule').on('click',function()
								{
									...
									$(':checkbox:checked').each(function()
									{
										...
										checked_app.push(appId);

										var userId=$(this).attr('data-user');
										checked_opt.push(userId);
									});
									var opt=checked_opt;
									var cand_app=$('#candidate_app').val(opt);
									var candidate_app=$('#candidate_app').val();
									var uid=candidate_app;

									var app=checked_app;
									var appId=$('#app_id').val(app);
									...
									$('#submit_date').on('click',function()
									{
										...
										$.ajax({
											url:'accepted_candidate.php',  
											method:'POST',  
											data:{app_id:app_id, candidate_app:candidate_app, int_date:int_date},  
											success:function(data){ 
												$('#disp_result').html('Interview date has been set to '+int_date); 
												window.location.href='schedule.php?uid='+uid;
											}
										})
									});
								});
							...
							<?php 
						        ...
						        $res=mysqli_query($conn,"SELECT * FROM applications WHERE recruiter_id='$SESSION' AND job_id='$id' AND status='accepted' ORDER BY percentage DESC");
